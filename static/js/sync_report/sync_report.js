...
var selected_city = "All";
/** Pragma Mark - Starting Points **/
var length_exam = [[10,15,20,25],[10,15,20,25]];
...
var updatePageContent = function() {
   ...
   $(document).ready(function() {
    ...
    sendPOSTRequest('/sync_report/sync_data/', {
        selected_city : selected_city

    }, function(response) {
     setTableData(response.data,response.code);
     if(response.data['bit'] == 1){
        $(".report-breadcrumb ").text(response.data['alert']);
 
         $( "div" ).remove( ".school" );

     }
     else{
        $(".report-breadcrumb ").text("You can see all synced schools with last sync date.");
     }
...
var refreshSchoolsDropdown = function() {
    sendPOSTRequest('/sync_report/sync_report_school/', {
...
var applyAndDismissSchoolDropdown = function() {
    ...
    if (node !== null && node.children == null) {
        ...
        selected_city = node.title;
        updatePageContent();
    } else {
        // a node is not selected
        toastr.warning('You must select perticular city to apply a filter');
    }
    toggleSchoolDropdown();
};
    ...
    if (code == 2001){
        ...
        try{
           ...
           columns : column,
            ...
            "targets": -1,
            "data": null,
            "defaultContent": "<button>History </button>"
        } ],
            ...
            "fnRowCallback": function( nRow, data, iDisplayIndex, iDisplayIndexFull ) {
                    if ( data[4] == "Active" )
                    {
                        $('td', nRow).css('background-color', '#d7e1f4');
                    }
            ...
            $('#data-table tbody').on( 'click', 'button', function () {
                var data1 = dataTable.row( $(this).parents('tr') ).data();
                $("#myModal").modal('show');
                $("#dates tr").remove();
                for (var i in data1[3]){
                    $('#dates').append('<tr><td>'+data1[3][i]+'</td></tr>');
                }
            } );
